...
size = 256
DATASET_PATH = f'/home/arnav/Disk/HistologyNet/monuseg_tiles_{size}x{size}'
        ...
        if os.path.isdir(wsi_folder_path):
            ...
            for scale_folder in scale_folders:
                ...
                for image_filename in image_filenames:
                    ...
                    image_hsv = rgb2hsv(np.array(image))

                    mask_s = (image_hsv[:, :, 1] >= 0.05)
                    mask_s_val = (np.sum(mask_s) / (size * size))

                    mask_v = (image_hsv[:, :, 2] <= 0.9)
                    mask_v_val = (np.sum(mask_v) / (size * size))
