...
def transnuss_trainer(args, model, snapshot_path):
    ...
    # db_pretrain = Monuseg_image_dataset(data_path=args.root_path_pretrain)
    ...
    train_loader = DataLoader(db_train_lab, batch_size=args.batch_size, shuffle=True, num_workers=1, pin_memory=True, worker_init_fn=worker_init_fn)
    validation_loader = DataLoader(db_val, batch_size=args.batch_size, shuffle=False, num_workers=1, pin_memory=True, worker_init_fn=worker_init_fn)
    ...
    iterator = tqdm(range(10,max_epoch), ncols=70)
    ...
    for epoch_num in iterator:
        ...
        if val_iou_score > best_val_iou:
            ...
            best_model_path = os.path.join(snapshot_path, 'best_model_pre.pth')
